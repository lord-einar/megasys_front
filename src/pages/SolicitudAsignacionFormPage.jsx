...
export default function SolicitudAsignacionFormPage() {
  ...
  const guardar = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...form,
        inventario_anterior_id: esReposicion ? (form.inventario_anterior_id || null) : null,
        denuncia_presentada: form.motivo === 'reposicion_robo' ? form.denuncia_presentada === 'true' : null
      }

      const res = await solicitudesAsignacionAPI.crear(payload)

      if (form.motivo === 'reposicion_robo' && adjuntos.denuncia) {
        await solicitudesAsignacionAPI.subirAdjunto(res.data.id, { tipo: 'denuncia', archivo: adjuntos.denuncia })
      }

      if (form.motivo === 'reposicion_rotura' && adjuntos.rotura) {
        await solicitudesAsignacionAPI.subirAdjunto(res.data.id, { tipo: 'rotura', archivo: adjuntos.rotura })
      }

      navigate(`/solicitudes-asignacion/${res.data.id}`)
    } catch (err) {
      ...
      <form onSubmit={guardar} className="card-base p-6 sm:p-8 max-w-4xl space-y-8">
        ...
        <section className="border-t border-surface-100 pt-8">
          ...
          <label className="block">
            ...
            <SelectBeneficiario
              value={form.beneficiario_personal_id}
              onChange={(id) => set('beneficiario_personal_id', id || '')}
              disabled={saving}
            />
          </label>
        </section>
        ...
        {esReposicion && (
          <section className="border-t border-surface-100 pt-8">
            <div className="mb-5">
              <h2 className="text-base font-bold text-surface-900">Equipo anterior</h2>
            ...
            <label className="block">
              ...
              <SelectInventarioAsignado
                ...
                value={form.inventario_anterior_id}
                onChange={(id) => set('inventario_anterior_id', id || '')}
                disabled={saving}
              />
            </label>
          </section>
            ...
            {form.motivo === 'reposicion_robo' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <label>
                  ...
                  <select
                    value={form.denuncia_presentada}
                    onChange={e => set('denuncia_presentada', e.target.value)}
                    className="input-base"
                  >
                ...
                <label>
                  ...
                  <input
                    type="file"
                    accept="image/*,.pdf"
                    onChange={e => setAdjuntos(prev => ({ ...prev, denuncia: e.target.files?.[0] || null }))}
                    className="input-base file:mr-4 file:rounded-lg file:border-0 file:bg-surface-100 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-surface-700"
                  />
                </label>
              </div>
            )}
            ...
            {form.motivo === 'reposicion_rotura' && (
              <label className="block">
                ...
                <input
                  ...
                  accept="image/*"
                  required
                  onChange={e => setAdjuntos(prev => ({ ...prev, rotura: e.target.files?.[0] || null }))}
                  className="input-base file:mr-4 file:rounded-lg file:border-0 file:bg-surface-100 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-surface-700"
                />
              </label>
            )}
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-surface-100 pt-6">
          <button type="button" onClick={() => navigate('/solicitudes-asignacion')} className="btn-secondary w-full sm:w-auto">
            Cancelar
          </button>
