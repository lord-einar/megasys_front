...
export default function ServiciosPage() {
  ...
  const eliminarServicio = async (servicio) => {
    const result = await Swal.fire({
      ...
      html: `Se eliminará el servicio <strong>${servicio.nombre}</strong>.`,
      icon: 'warning',
  ...
  return (
    <div className="p-6 sm:p-8 bg-surface-50 min-h-screen animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Servicios Contratados</h1>
          <p className="text-surface-500 mt-1 font-medium">Catálogo de servicios disponibles por proveedor</p>
        </div>
      ...
      <div className="card-base p-6 bg-white border border-surface-200 shadow-sm mb-8 space-y-4">
        <form onSubmit={handleBuscar} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-surface-100">
            ...
            <div>
              <label className="block text-xs font-bold text-surface-500 uppercase mb-1.5">Filtrar por Tipo</label>
              <div className="relative">
                <select
      ...
      {loading ? (
        <div className="p-12 text-center">
          ...
          <p className="text-surface-500 font-medium">Cargando servicios...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-surface-200 border-dashed">
          ...
          <p className="text-surface-900 font-bold text-lg">No se encontraron servicios</p>
              ...
              >
                <div className="p-6 border-b border-surface-100 bg-surface-50/50">
                  ...
                  {servicio.id_servicio && (
                    <p className="text-[10px] font-mono text-surface-400 uppercase tracking-widest bg-white w-fit px-1.5 py-0.5 rounded border border-surface-100">
                ...
                <div className="p-6 space-y-4 flex-1">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-surface-500 uppercase tracking-wide">Proveedor</p>
                      <p
                        className="text-surface-700 text-sm font-medium hover:text-primary-600 cursor-pointer transition-colors"
                        onClick={() => navigate(`/proveedores/${servicio.proveedor_id}`)}
                      >
                        {servicio.proveedor?.empresa || 'N/A'}
                      </p>
                    </div>
                  </div>
                  ...
                  {servicio.descripcion && (
                    <div className="p-3 bg-surface-50 rounded-xl border border-surface-100">
                      <p className="text-xs text-surface-600 line-clamp-3 italic">"{servicio.descripcion}"</p>
                    </div>
                  )}
                </div>
