  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        if (!personal) {
          Swal.fire('Error', 'No se pudo cargar los datos del personal', 'error')
          return
        }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (errorData.fields && Object.keys(errorData.fields).length > 0) {
        ...
        Swal.fire({
          title: 'Error',
          text: errorData.general || 'Error al actualizar el personal',
          icon: 'error',
          customClass: { popup: 'rounded-2xl' }
        })
      }
    ...
    <div className="p-6 sm:p-8 bg-surface-50 min-h-screen animate-fade-in">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Editar Personal</h1>
            <p className="text-surface-500 mt-1 font-medium">Modifica la información del miembro del equipo</p>
          </div>
          <button
            type="button"
        ...
        <LoadingOverlay isVisible={isLoading || isSubmitting} message="Guardando cambios..." />
      </div>
    </div>
  )
}
