...
import { tipoArticuloAPI } from '../services/api'
import { usePermissions } from '../hooks/usePermissions'
import { useListData } from '../hooks/useListData'
import { usePermissionError } from '../hooks/usePermissionError'
import { getPaginationNumbers } from '../utils/paginationHelper'
import Swal from 'sweetalert2'
import ModalTipoArticulo from '../components/inventario/ModalTipoArticulo'
...
export default function TiposArticuloPage() {
  ...
  const {
    ...
    nextPage,
    reload
  } = useListData(tipoArticuloAPI.list, {
    initialLimit: 12,
  ...
  const handleSave = async (data) => {
    try {
      if (selectedItem) {
        await tipoArticuloAPI.update(selectedItem.id, data)
        Swal.fire({
          title: 'Actualizado',
          text: 'El tipo de artículo ha sido actualizado.',
          icon: 'success',
          timer: 1500,
          customClass: { popup: 'rounded-2xl' }
        })
      } else {
        await tipoArticuloAPI.create(data)
        Swal.fire({
          title: 'Creado',
          text: 'El tipo de artículo ha sido creado.',
          icon: 'success',
          timer: 1500,
          customClass: { popup: 'rounded-2xl' }
        })
      ...
      Swal.fire({
        title: 'Error',
        text: err.message || 'No se pudo guardar el tipo de artículo',
        icon: 'error',
        customClass: { popup: 'rounded-2xl' }
      })
  ...
  const eliminarTipo = async (tipo) => {
    ...
    if (result.isConfirmed) {
      try {
        await tipoArticuloAPI.delete(tipo.id)
        await Swal.fire({
          title: 'Eliminado',
          text: 'El tipo de artículo ha sido eliminado correctamente.',
          icon: 'success',
          timer: 1500,
          customClass: { popup: 'rounded-2xl' }
        })
        ...
        Swal.fire({
          title: 'Error',
          text: err.message || 'No se pudo eliminar el tipo de artículo',
          icon: 'error',
          customClass: { popup: 'rounded-2xl' }
        })
      }
  ...
  return (
    <div className="p-6 sm:p-8 bg-surface-50 min-h-screen animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Tipos de Artículo</h1>
          <p className="text-surface-500 mt-1 font-medium">Categorías de clasificación de artículos de inventario</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-surface-200 border-dashed">
          ...
          <p className="text-surface-900 font-bold text-lg">No se encontraron tipos de artículo</p>
              ...
              >
                ...
                <div className="p-4 bg-surface-50 border-t border-surface-100 flex gap-2">
                  <button
                    ...
                    className={`flex-1 px-3 py-2 border rounded-lg transition-all text-xs font-bold shadow-sm flex items-center justify-center gap-2 ${canUpdate('inventario')
